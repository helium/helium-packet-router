...
-spec(init(Args :: term()) ->
  {ok, State :: #hpr_gwmp_client_state{}} | {ok, State :: #hpr_gwmp_client_state{}, timeout() | hibernate} |
...
-spec(terminate(Reason :: (normal | shutdown | {shutdown, term()} | term()),
    State :: #hpr_gwmp_client_state{}) -> term()).
terminate(_Reason, _State = #hpr_gwmp_client_state{}) ->
  ok.

%% @private
%% @doc Convert process state when code is changed
-spec(code_change(OldVsn :: term() | {down, term()}, State :: #hpr_gwmp_client_state{},
    Extra :: term()) ->
