-module(hpr_test_ics_gateway_service).
...
-export([
    register_gateway_location/2
]).

-define(KNOWN_LOCS, hpr_test_ics_gateway_service_known_locations).
...
verify_location_req(Req) ->
    ...
    libp2p_crypto:verify(
        ...
        libp2p_crypto:bin_to_pubkey(hpr_utils:pubkey_bin())
    ).
...
register_gateway_location(PubKeyBin, Location) ->
    Map = persistent_term:get(?KNOWN_LOCS, #{}),
    ok = persistent_term:put(?KNOWN_LOCS, Map#{PubKeyBin => Location}).
