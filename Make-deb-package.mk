...
# TODO accommodate results of `rebar3 as ${BUILD_NET} release` e.g., mainnet
# FIXME: just rel/ and below fails, but same with all relx-generated subdirectories
# regardless of .deb or generic VM with /vagrant mapped to either location.
#REBAR_BUILD_DIR := _build/default
#REBAR_RELEASE_DIR := ${REBAR_BUILD_DIR}/rel/${SNAKE_NAME}
...
package-bin:
	rm -rf _release
	([ "${DEST}" ] && mkdir -p _release${DEST}/log) || true
...
vm-test:
	...
	[ -f _release/vm/${SHORT_NAME}.deb ] || \
	  ln "${DEBIAN_PACKAGE_NAME}_${REVISION}_${DEB_ARCH}.deb" \
	    _release/vm/${SHORT_NAME}.deb
	...
	(cd _release/vm/ && vagrant ssh -c 'sudo systemctl status ${SHORT_NAME}')
...
# For debugging systemd & init.d startup
vm-ssh:
	(cd _release/vm/ && vagrant ssh)

# For confirming bits generated by `relx`
vm-sanity-check:
	sed 's/^\s*apt[- ].*$$//' \
